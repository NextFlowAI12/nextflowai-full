// _app.js robusto: carrega CSS e evita ecrã branco se houver erro
import '../styles/globals.css';
import '../styles/styles.css';
import React, { useEffect } from 'react';

function ErrorFallback({ error }){
  useEffect(()=>{ if(error) console.error('App crash:', error); }, [error]);
  return (
    <div className="container section">
      <div className="card" style={{borderColor:'rgba(248,113,113,.35)'}}>
        <div className="title">Ocorreu um erro no carregamento</div>
        <p className="small">Recarrega a página. Se continuar, envia esta mensagem ao suporte:</p>
...
class ErrorBoundary extends React.Component{
  ...
  render(){ return this.state.hasError ? <ErrorFallback error={this.state.error}/> : this.props.children; }
}
